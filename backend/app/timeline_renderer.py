...
def render_timeline(project_id: str, asset_id: str, name: str, clips: list[RenderClip]) -> dict[str, object]:
    if not clips:
    ...
    with tempfile.TemporaryDirectory(prefix="amplifier-render-") as directory:
        ...
        command = _ffmpeg_command(clips, inputs, [_has_audio(path) for path in inputs], output, duration, input_indexes)
...
def _ffmpeg_command(clips: list[RenderClip], inputs: list[Path], has_audio: list[bool], output: Path, duration: float, input_indexes: list[int] | None = None) -> list[str]:
