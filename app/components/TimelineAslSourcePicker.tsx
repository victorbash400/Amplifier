import { ChevronDown, FileText, Languages, Video } from "lucide-react";
import { useEffect, useRef, useState } from "react";
import { createPortal } from "react-dom";
import styles from "./TimelineAslSourcePicker.module.css";

export type AslSource = "transcript" | "description";
...
export function TimelineAslSourcePicker({ disabled, onSelect, working }: { disabled: boolean; onSelect: (source: AslSource) => void; working: boolean }) {
  ...
  return <><button aria-expanded={open} aria-haspopup="menu" aria-label="ASL interpretation" aria-pressed={working || undefined} className={styles.trigger} disabled={disabled} onClick={toggle} ref={button} title={disabled ? "Select a video clip to use ASL interpretation" : "ASL interpretation"} type="button"><Languages size={15} /><span>ASL</span><ChevronDown className={styles.chevron} size={8} /></button>{open && createPortal(<section aria-label="Generate ASL from" className={styles.menu} ref={menu} role="menu" style={position}><strong>Generate from</strong><button onClick={() => select("transcript")} role="menuitem" type="button"><FileText size={14} /><span>Transcript</span></button><button onClick={() => select("description")} role="menuitem" type="button"><Video size={14} /><span>Video description</span></button></section>, document.body)}</>;
}
