...
import { ChevronDown, ChevronRight, FileUp, FolderPlus, Search } from "lucide-react";
...
export function FileSidebar({ project, folders, files, onFoldersChange, onFilesChange, onOpenFile }: FileSidebarProps) {
  ...
  return <aside className={styles.sidebar} aria-label="Assets"><section className={styles.search}><Search size={14} /><input aria-label="Search assets" onChange={(event) => setQuery(event.target.value)} placeholder="Search" value={query} /></section><nav className={styles.actions} aria-label="Asset actions"><label aria-label="Upload files" title="Upload files"><FileUp size={16} /><input multiple onChange={upload} type="file" /></label><button aria-label="New folder" onClick={() => createFolder()} title="New folder" type="button"><FolderPlus size={16} /></button></nav><nav className={styles.tree} aria-label="Asset tree"><p className={styles.folderRow} data-selected={selectedId === "root"}><button aria-label={`${rootExpanded ? "Collapse" : "Expand"} ${project.name}`} className={styles.chevron} onClick={() => toggle("root")} type="button">{rootExpanded ? <ChevronDown size={14} /> : <ChevronRight size={14} />}</button><button className={styles.folderName} onClick={() => setSelectedId("root")} type="button"><FolderIcon color={project.color} /><strong>{project.name}</strong></button><button aria-label="New folder in project" className={styles.rowAction} onClick={() => createFolder("root")} type="button"><FolderPlus size={14} /></button></p>{rootExpanded && visibleFiles.filter((file) => file.folderId === "root").map((file) => <FileRow file={file} key={file.id} onDelete={(id) => onFilesChange(files.filter((item) => item.id !== id))} onOpen={onOpenFile} />)}{rootExpanded && visibleFolders.filter((folder) => !folder.parentId).map((folder) => <FolderRow expanded={expanded} files={visibleFiles} folder={folder} folders={visibleFolders} key={folder.id} onCreateFolder={createFolder} onDeleteFile={(id) => onFilesChange(files.filter((item) => item.id !== id))} onDeleteFolder={deleteFolder} onOpenFile={onOpenFile} onSelect={setSelectedId} onToggle={toggle} selectedId={selectedId} />)}</nav></aside>;
}
