...
export function CreatorMessageBubble({ message }: { message: CreatorMessage }) {
  return <article className={styles.message} data-role={message.role}>{message.blocks.map((block) => block.kind === "reasoning" ? <CreatorReasoningBlock block={block} key={block.id} /> : block.kind === "tool" ? <CreatorToolCallIndicator key={block.id} tool={block.tool} /> : block.kind === "timeline-shot" ? <TimelineShotAttachment key={block.id} shot={block.shot} /> : <CreatorMarkdown content={block.content} key={block.id} />)}</article>;
}
