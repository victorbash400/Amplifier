...
export async function POST(request: Request) {
  const body = await request.json() as { agentId?: string; projectId?: string; sessionId?: string; message?: string };
  ...
  const response = await fetch(`${backendUrl.replace(/\/$/, "")}/agent/chat`, {
    ...
    body: JSON.stringify({ agent_id: body.agentId || "general", user_id: "local-user", session_id: body.sessionId, message }),
