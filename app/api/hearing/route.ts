...
export async function POST(request: Request) {
  ...
  if (body.action === "noise-reduce") {
    ...
    const response = await fetch(`${backendUrl.replace(/\/$/, "")}/hearing/noise-reduce`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ project_id: body.projectId, asset_id: body.assetId, source_asset_id: body.sourceAssetId, source_object_key: body.sourceObjectKey, source_name: body.sourceName, content_type: body.contentType, folder_id: body.folderId, strength: body.strength, duration: body.duration }) });
  ...
  const response = await fetch(`${backendUrl.replace(/\/$/, "")}/hearing/asl`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ project_id: body.projectId, asset_id: body.assetId, source_object_key: body.sourceObjectKey, start: body.start, end: body.end, source: body.source, cues: body.cues }) });
