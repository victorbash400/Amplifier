...
export async function POST(request: Request) {
  ...
  const response = await fetch(`${backendUrl.replace(/\/$/, "")}/vision/${filter ? "filter" : "narration"}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ project_id: body.projectId, asset_id: body.assetId, source_asset_id: body.sourceAssetId, source_object_key: body.sourceObjectKey, source_name: body.sourceName, content_type: body.contentType, folder_id: body.folderId, action: body.action, preset: body.preset, start: body.start, end: body.end }) });
